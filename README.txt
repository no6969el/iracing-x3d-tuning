...
Before: Pre-Race-Quiet     After: Post-Race-Restore
  ...
  It reports three verdicts:
     RACE-QUIET ACTIVE  services down AND every visible task disabled
     PARTLY QUIET       services down, but tasks can still fire. It lists
                        each one by full path. Re-run Pre-Race-Quiet as
                        admin; if they survive that, run Trace-QuietReverts
                        elevated to find what is holding them on.
     NOT QUIETED        nothing has been done yet

  Tasks that don't exist on your Windows build are counted separately and
...
WHAT GETS DISABLED BEFORE A RACE  (v3.3.0)
  ...
  Tasks (39):
     ...
     UpdateOrchestrator\Schedule Scan, Schedule Scan Static Task,
       ...
       Schedule Work, Start Oobe Expedite Work
     InstallService\ScanForUpdates, ScanForUpdatesAsUser
     PushToInstall\LoginCheck, Registration
     LanguageComponentsInstaller\ReconcileLanguageResources
     Flighting\FeatureConfig\ReconcileFeatures, UsageDataReceiver,
       UsageDataFlushing
